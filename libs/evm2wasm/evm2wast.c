...
static char *assemble_segments(size_t *segments, size_t len)
{
	...
	char *ret = malloc(len*(8+digits(len)+strlen(wasm)+1) + 200);
	if(ret == 0) goto err;
...
static char *add_stack_check(char **segment)
{
	...
	char *ret = malloc(strlen(check) + strlen(*segment)+1);
...
char *build_module(const char *wast)
{
	...
	char *ret = malloc(strlen(wast) + gadgets_len+1024);
	if(!ret) return 0;
			...
			case PUSH32:
				{
					...
					if(!bytes) goto err;
					int bytes_rounded = (evm_code[i-1] + 7) >> 3;
					char *push = 0;
					int q;
