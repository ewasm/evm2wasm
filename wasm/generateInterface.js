...
function generateManifest (interfaceManifest) {
  const json = {}
  for (let opcode in interfaceManifest) {
    ...
    (i64.store (i32.add (get_global $sp) (i32.const ${spOffset * 32 + 8 * 2})) (i64.const 0))`
      ...
      if (!op.async) {
        call += '(call $bswap_m128 (i32.add (i32.const 32)(get_global $sp))) drop'
      }
      ...
      call += `)
    (call $bswap_m160 (i32.add (get_global $sp) (i32.const ${spOffset * 32})))
    drop
    ;; zero out mem
    (i64.store (i32.add (get_global $sp) (i32.const ${spOffset * 32 + 8 * 3})) (i64.const 0))
    ...
    (i32.const ${spOffset * 32}))`
      ...
      call += `)
      (call $bswap_m256 (i32.add (i32.const 32) (get_global $sp)))
      drop
      `
    } else if (output === 'i32') {
      if (op.async) {
...
const json = generateManifest(interfaceManifest)
// add math ops
